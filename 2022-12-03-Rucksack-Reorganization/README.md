...
```
go run rucksack-part1.go test.txt
go run rucksack-part1.go
```
...
he only way to tell which item type is the right one is by finding the one item type that is common between all three Elves in each group.
...
```
go run rucksack-part2.go test.txt
go run rucksack-part2.go
```
